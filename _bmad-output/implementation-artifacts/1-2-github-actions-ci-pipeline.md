...
| 2026-05-06 | Phase 4 simplify pass: no-op (reviewed 2 files across reuse / quality / efficiency layers, all three converged on "ship as-is"). Reuse: no existing helper duplicates `_load_workflow` / `_on_field` / `_step_names_and_runs`; conftest.py is empty placeholder; project convention (S01/S04/S06/S11) is to inline `REPO_ROOT` + parse, not centralize — Rule of Three not met. Quality: `_on_field` PyYAML True-key comment is genuine WHY (non-obvious quirk); `_step_names_and_runs` returning tuples is clearer than dicts at the destructuring callsites; the if/elif/raise on `on:` shape is a justified exhaustiveness guard. The one borderline (a `COVERAGE_GATE = "--cov-fail-under=85"` constant in the test) is cosmetic and below the simplify bar. Efficiency: `_load_workflow` 3× re-read is sub-ms and pytest-idiomatic (test isolation > caching); splitting CI into parallel lint/mypy/pytest jobs would re-pay ~30–90 s setup per job vs <10 s of work — net loss; `ruff check .` scope is correctly narrowed by ruff defaults (skips non-Python under `_bmad-output/` and `.github/`). No follow-up commit needed. |
