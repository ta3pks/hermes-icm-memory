...
| 2026-05-06 | Phase 4 / `/simplify` (Reuse + Quality + Efficiency reviewers in parallel). Net −48 lines across four files, no behaviour change. Actioned: (a) exposed `IMPORTANCE_CHOICES` publicly from `config.py`; `tools.py` now derives both `_VALID_IMPORTANCE` and the `icm_store` schema enum from it (single source of truth — eliminates drift between three previous duplicates); (b) new `_positive_int` helper centralizes the bool-rejecting positive-int check (was duplicated 3×); (c) extracted `_run_read(provider, name, payload_key, do_call)` to fold the shared guard + try/except/log/degrade shape across the three read handlers — recall passes a closure since its positional signature is `(query, limit, db, ms)` while topics/health lead with `(db, ms)`; (d) stripped story-tracking residue (AD/FR/AC tags, "Phase 3 finding", reviewer names) from production code docstrings/comments and test section headers; (e) collapsed three near-identical `test_*_failure_*` tests + four importance-fallback tests into two parametrized tests; (f) dropped the dead `task: tuple[str, str, str, list[str]] = ...` annotation. Skipped (documented): `copy.deepcopy` of schemas (sub-ms, not hot path), full deduping of dispatch keys via schema lookup (adds indirection across 4 stable strings), three-helper unification (different validators). Final: 124 passed / 3 skipped; tools.py 100% line+branch; package 97.28%; ruff + mypy --strict clean. |
