...
| 2026-05-06 | Phase 4 simplify pass: applied the LOW finding — hoisted `_SCHEMA_BY_KEY` to a module-level `Final` dict built once at import, removed the per-call dict comprehension inside `validate()`. config.py drops 3 lines, coverage stays at 100 % line+branch (gate 85 %), ruff + mypy --strict still clean, all 15 test cases still pass. No other reuse / quality / efficiency findings — `copy.deepcopy` on `_SCHEMA_ENTRIES` is necessary (callers may mutate); the `try/except Exception` outer wrapper in `validate()` is mandated by AD-18; the four-branch `_validate_one` dispatch is clearer than a dispatch dict for four cases. |
